...
def is_swap(player_score, opponent_score):
    ...
    """
    # BEGIN PROBLEM 4
    # assert player_score < 100 and opponent_score < 100, 'The game should be over.'
    excitement = 3**(player_score + opponent_score)
    last_digit = excitement % 10
    while excitement >= 10:
        excitement //= 10
    first_digit = excitement
...
def play(strategy0,
    ...
    while score0 < goal and score1 < goal:
        if who == 0:
            ...
            score0 += points
            if feral_hogs == True:
                score0 += execute_feral_hogs(num_rolls, previous_scored0)
                previous_scored0 = points
        else:
            num_rolls = strategy1(score1, score0)
            points = take_turn(num_rolls, score0, dice)
            score1 += points
            if feral_hogs == True:
                score1 += execute_feral_hogs(num_rolls, previous_scored1)
                previous_scored1 = points
...
def execute_feral_hogs(num_rolls, previous_scored):
    # if num_rolls is abs value 2 away from the points scored from the previous round
    if abs(num_rolls - previous_scored) == 2:
        return 3
    return 0
...
def announce_highest(who, prev_high=0, prev_score=0):
    ...
    def say(score0, score1):
        if who == 0:
            ...
            highest_gain = prev_high
            if prev_high < gain:
                print(gain, "point(s)! That's the biggest gain yet for Player", who)
                highest_gain = gain
            return announce_highest(who, highest_gain, score0)
        else:
            ...
            if prev_high < gain:
                print(gain, "point(s)! That's the biggest gain yet for Player", who)
                highest_gain = gain
            return announce_highest(who, highest_gain, score1)
    return say
    # END PROBLEM 7
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    score_after_free_bacon = score + free_bacon(opponent_score)
    if is_swap(score_after_free_bacon, opponent_score):
        if opponent_score > score_after_free_bacon:
            return 0
...
s0, s1 = play(always(5), always(3), score0=91, score1=10,
              dice=always_three, feral_hogs=False)
print(s0, s1)
